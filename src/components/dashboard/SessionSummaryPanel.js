...
export default function SessionSummaryPanel({ session, sourceLabel }) {
  ...
  const isActive = sourceLabel === "Active Session";

  return (
    <section className="rounded-2xl border border-white/10 bg-slate-950/40 p-5 shadow-2xl backdrop-blur-xl">
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-bold uppercase tracking-wider text-white">Overall Session Summary</h2>
          <p className="mt-1 text-xs text-slate-500">
            {isActive ? "Provisional information only. Structured summary is generated after finishing." : "Structured summary generated by the session domain layer."}
          </p>
        </div>
      ...
      {sections.length === 0 ? (
        <div className="rounded-xl border border-dashed border-white/10 bg-slate-900/30 p-5 text-sm text-slate-400">
          {session ? "A structured summary will appear after the session is finished." : "No completed session summary is available yet."}
        </div>
      ) : (
