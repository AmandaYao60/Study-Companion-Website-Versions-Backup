    ...
    <section className="rounded-2xl border border-white/10 bg-slate-950/40 p-5 shadow-2xl backdrop-blur-xl">
      ...
      {rows.length === 0 ? (
        ...
        <div className="max-h-80 overflow-y-auto rounded-xl border border-white/10">
          <div className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3 border-b border-white/10 bg-slate-900/70 px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-slate-500">
            <span>Date & Time</span>
          ...
          <div className="divide-y divide-white/10">
            {rows.map((row) => (
              <button
                ...
                className="group grid w-full grid-cols-[1fr_1fr_1fr_auto] gap-3 px-4 py-3 text-left text-xs text-slate-300 transition-all hover:bg-cyan-400/[0.08] focus:bg-cyan-400/[0.1] focus:outline-none focus:ring-2 focus:ring-cyan-400/40"
              >
                ...
                <span className="text-right text-cyan-300 opacity-80 transition-all group-hover:opacity-100">View details &gt;</span>
              </button>
            ))}
          </div>
        </div>
