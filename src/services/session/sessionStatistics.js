import { DATA_QUALITY, DEFAULT_METRIC_TREND_THRESHOLDS, EMOTION_LABELS, METRIC_TREND } from "./sessionConstants.js";
...
export const calculateSessionStatistics = (metricSamples = [], session = {}, options = {}) => {
  ...
  return {
    attention: calculateMetricStatistics(samples.map((sample) => sample.attention), { ...options, meaningfulTrendChange: DEFAULT_METRIC_TREND_THRESHOLDS.attention }),
    fatigue: calculateMetricStatistics(samples.map((sample) => sample.fatigue), { ...options, meaningfulTrendChange: DEFAULT_METRIC_TREND_THRESHOLDS.fatigue }),
    valence: calculateMetricStatistics(samples.map((sample) => sample.valence), { ...options, meaningfulTrendChange: DEFAULT_METRIC_TREND_THRESHOLDS.valence }),
    arousal: calculateMetricStatistics(samples.map((sample) => sample.arousal), { ...options, meaningfulTrendChange: DEFAULT_METRIC_TREND_THRESHOLDS.arousal }),
    dominantEmotion,
